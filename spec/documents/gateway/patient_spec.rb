...
describe Documents::Gateway::Patient do
  ...
  shared_examples 'the_patient_identity_set_is_in_error' do
    ...
    it 'raises a patient set in error exception' do
      allow(Documents::Api::Client).to receive(:perform_request).and_return(mock_response)
      expect {
  ...
  describe '.active_identities' do
    ...
    context 'when a patient exists with root: root42 and extension: ext42' do
      ...
      context 'and querying for root: root42, extension: ext42' do
        ...
        it 'is the identities returned from Documents as a hash' do
          WebMock.stub_request(
            :get,
  ...
  describe 'self.delete' do
    ...
    it 'performs a request returning 403 - requesting application or tenant is not authorized to perform lookup with Patient Identity' do
      allow(Documents::Api::Client).to receive(:perform_request).and_return(double('Mock Response', code: 403, body: { message: 'Application is not authorized to perform lookup with Patient Identity' }))
    ...
    it 'performs a request returning 404 - requested identity does not exist in the system' do
      allow(Documents::Api::Client).to receive(:perform_request).and_return(double('Mock Response', code: 404, body: {}))
    ...
    it 'performs a request returning 409 - requested identity is not "in error"' do
      allow(Documents::Api::Client).to receive(:perform_request).and_return(double('Mock Response', code: '409', body: '{ "error": "Patient Identity is not in Error" }' ))
    ...
    it 'performs a request returning 409 - requested identity has documents' do
      allow(Documents::Api::Client).to receive(:perform_request).and_return(double('Mock Response', code: '409', body: '{ "error": "Patient Identity has documents" }'))
  ...
  describe 'self.request' do
    ...
    it 'raises an patient identity unauthorized error if request is unauthorized' do
      expect {
        allow(Documents::Api::Client).to receive(:perform_request).and_return(unauthorized_response)
